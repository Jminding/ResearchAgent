...
class SubagentTracker:
    ...
    def __init__(self, transcript_writer=None, session_dir: Optional[Path] = None):
        ...
        # Agent prompts log (full prompts given to each agent)
        self.prompts_log_file = None
        if session_dir:
            tool_log_path = session_dir / "tool_calls.jsonl"
            self.tool_log_file = open(tool_log_path, "w", encoding="utf-8")

            prompts_log_path = session_dir / "agent_prompts.txt"
            self.prompts_log_file = open(prompts_log_path, "w", encoding="utf-8")
    ...
    def register_subagent_spawn(
        ...
        # Log full prompt to prompts file
        if self.prompts_log_file:
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            separator = "="*80
            self.prompts_log_file.write(f"\n{separator}\n")
            self.prompts_log_file.write(f"AGENT: {subagent_id}\n")
            self.prompts_log_file.write(f"TYPE: {subagent_type}\n")
            self.prompts_log_file.write(f"SPAWNED: {timestamp}\n")
            self.prompts_log_file.write(f"DESCRIPTION: {description}\n")
            self.prompts_log_file.write(f"{separator}\n\n")
            self.prompts_log_file.write(f"PROMPT:\n{prompt}\n\n")
            self.prompts_log_file.flush()
    ...
    async def pre_tool_use_hook(self, hook_input, tool_use_id, context):
        ...
        if is_subagent:
            ...
            # If this is a Task tool (agent spawning another agent), log the prompt
            if tool_name == 'Task' and self.prompts_log_file and tool_input:
                self._log_task_prompt(agent_id, tool_input, timestamp)
            ...
            # If main agent spawns a Task, log it too
            if tool_name == 'Task' and self.prompts_log_file and tool_input:
                self._log_task_prompt("MAIN_AGENT", tool_input, timestamp)
    ...
    def _log_task_prompt(self, spawner_agent_id: str, tool_input: Dict[str, Any], timestamp: str):
        """
        Log Task tool prompts to the prompts file.

        Args:
            spawner_agent_id: ID of agent spawning the task
            tool_input: Task tool input containing prompt, subagent_type, description
            timestamp: ISO timestamp of the spawn
        """
        if not self.prompts_log_file:
            return
        ...
        self.prompts_log_file.write(f"\n{separator}\n")
        self.prompts_log_file.write(f"SPAWNER: {spawner_agent_id}\n")
        self.prompts_log_file.write(f"TARGET AGENT TYPE: {subagent_type}\n")
        self.prompts_log_file.write(f"TIME: {formatted_time}\n")
        self.prompts_log_file.write(f"DESCRIPTION: {description}\n")
        self.prompts_log_file.write(f"{separator}\n\n")
        self.prompts_log_file.write(f"PROMPT:\n{prompt}\n\n")
        self.prompts_log_file.flush()
